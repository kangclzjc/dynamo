...
def test_scheduling_rejects_non_positive_timeouts():
    with pytest.raises(ValidationError):
        SchedulingConfig(request_timeout_seconds=0)
    with pytest.raises(ValidationError):
        SchedulingConfig(request_timeout_seconds=-1)
    with pytest.raises(ValidationError):
...
def test_planner_config_without_scheduling_section_loads_unchanged():
    ...
    assert pc.scheduling.use_orchestrator is False
    assert pc.scheduling.request_timeout_seconds == 5.0
...
def test_planner_config_with_scheduling_override_parses():
    ...
    assert pc.scheduling.request_timeout_seconds == 2.5
