...
from dynamo.planner.plugins.orchestrator.internal_register import register_internal
...
def test_module_level_register_internal_equivalent(ctx_factory):
    ctx = ctx_factory()
    register_internal(
        ctx["orchestrator"],
        "stub",
        "propose",
        10,
        StubPlugin(propose=_make_propose_stub(5)),
        is_builtin=False,
    )
...
async def test_tick_empty_baseline_and_no_plugins_is_skip_no_targets(
    ...
    # M-4 skip_no_targets + audit event.
    ctx = ctx_factory()
    outcome = await ctx["orchestrator"].tick(PipelineContext(), baseline={})
